...
function initComponents({
	...
	if (typeof window.theme.header === 'undefined') {
		if (window.location.pathname === '/rhye-creative-ajax-html5-template-2023-11-27-05-22-53-utc/HTML/home.html') {
			...
			// Initialize standard Header for all other pages
			window.theme.header = new Header();
			console.log("Initialized standard Header");
		}
	}
...
class HeaderHero {
		constructor() {
			...
			this.$curtain = $('#js-header-curtain');
			this.$curtainTransition = $('#js-header-curtain-transition');
			this.$overlay = $('.mg-header__wrapper-overlay-menu');
			this.burgerOpenClass = 'header__burger_opened';
			this.$headerColumns = this.$header.find('.header__col');
			this.$headerLeft = this.$header.find('.header__col-left');
			this.$overlayWidgets = this.$header.find('.header__wrapper-overlay-widgets');
			...
			on: {
				slideChange: function () {
					...
					if (this.activeIndex === 0) {
						// Move header to the bottom for the first slide
						header.classList.add("header--bottom");
						header.classList.remove("header--top");
					} else {
						// Move header to the top for other slides
			...
			on: {
				slideChangeTransitionStart: function () {
					...
					if (this.activeIndex === 0) {
						...
						subElement.classList.remove('pad-zero');
					} else {
						...
						subElement.classList.add('pad-zero');
					}
				}
			}
		});
...
// For navigation between pages without full reload
...
// If you're using any form of AJAX navigation
// Add this listener if applicable to your site structure
window.addEventListener('popstate', updatePadding);